...
const Overview = ({ match, map, players }: IProps) => {
  ...
  useOnConfigChange(
    ...
    async (data) => {
      console.log(data);
      if (
        !data?.match_preview?.match?.left.id ||
        !data?.match_preview?.match?.right.id
      )
        return;

      const teams = await Promise.all([
        api.teams.getTeam(data?.match_preview?.match?.left.id),
        api.teams.getTeam(data?.match_preview?.match?.right.id),
      ]);
    ...
    <>
      {playerData ? (
        <PlayerOverview
          ...
          player={playerData}
          players={players}
          show={!!previewData.player_preview_toggle}
          veto={veto}
        />
      ) : null}
      {matchData && teams[0] && teams[1] ? (
        <MatchOverview
          match={matchData}
          veto={veto}
          teams={teams}
          show={!!previewData.match_preview_toggle}
        />
      ) : null}
    </>
  );
